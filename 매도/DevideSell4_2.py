...
def sell_stock(code):
    ...
    try:
        if code in sell_List: 
            ...
            stocks_qty = []
            for i in range(dv): 
                stocks_qty.append(stock_qty // dv)
                #if stocks_qty[i] == 0:
                #    stocks_qty.append(i, stock_qty) 
                #if stock_qty == 0:
                #    return False
                #printlog('bought_list:', bought_list, 'len(bought_list):',
                #    len(bought_list), 'target_buy_count:', target_buy_count)     
                if  target_price * 0.9 + ((i+1) * target_price * 0.1) > current_price > target_price * 0.9 + (i * target_price * 0.1) : 
                    printlog(stock_name + '(' + str(code) + ') ' + str(stocks_qty[i]) +
                    ...
                    cpOrder.SetInputValue(2, accFlag[0]) # 상품구분 - 주식 상품 중 첫번째
                    cpOrder.SetInputValue(3, code)       # 종목코드
                    cpOrder.SetInputValue(4, stocks_qty[i])    # 매수할 수량
                    cpOrder.SetInputValue(7, "2")        # 주문조건 0:기본, 1:IOC, 2:FOK
                    cpOrder.SetInputValue(8, "12")       # 주문호가 1:보통, 3:시장가
                                                        # 5:조건부, 12:최유리, 13:최우선 
                    # 매도 주문 요청
                    ret = cpOrder.BlockRequest() 
                    printlog('최유리 FOK 매도 ->', stock_name, code, stocks_qty[i], '->', ret)
                    if ret == 4:
                        remain_time = cpStatus.LimitRequestRemainTime
                        printlog('주의: 연속 주문 제한에 걸림. 대기 시간:', remain_time/1000)
                        time.sleep(remain_time/1000) 
                    ...
                    printlog('get_stock_balance :', stock_name, stocks_qty[i])
                    if stock_qty == 0:
                        sell_List.remove(code)
                    dbgout("`sell_stock("+ str(stock_name) + ' : ' + str(code) + 
                        ") -> " + str(stocks_qty[i]) + " EA sell!" + "`")
