...
def sell_stock(code):
    ...
    try:
        if code in sell_List: 
            ...
            stock_name, stock_qty = get_stock_balance(code)  # 종목명과 보유수량 조회
            stock_qty2 = stock_qty // 2
            stock_qty3 = stock_qty - stock_qty2
            #printlog('bought_list:', bought_list, 'len(bought_list):',
            #    len(bought_list), 'target_buy_count:', target_buy_count)     
            if target_price * 1.0 > current_price > target_price * 0.9 or current_price < target_price * 0.8: 
                ...
                cpOrder.SetInputValue(2, accFlag[0]) # 상품구분 - 주식 상품 중 첫번째
                cpOrder.SetInputValue(3, code)       # 종목코드
                cpOrder.SetInputValue(4, stock_qty2)    # 매수할 수량
                cpOrder.SetInputValue(7, "1")        # 주문조건 0:기본, 1:IOC, 2:FOK
                cpOrder.SetInputValue(8, "12")       # 주문호가 1:보통, 3:시장가
                                                    # 5:조건부, 12:최유리, 13:최우선 
                # 매수 주문 요청
                ret = cpOrder.BlockRequest() 
                printlog('최유리 IOC 매수 ->', stock_name, code, stock_qty2, '->', ret)
                if ret == 4:
                    remain_time = cpStatus.LimitRequestRemainTime
                    printlog('주의: 연속 주문 제한에 걸림. 대기 시간:', remain_time/1000)
                    time.sleep(remain_time/1000) 
            ...
            if current_price > target_price * 1.0 or current_price < target_price * 0.9: 
                printlog(stock_name + '(' + str(code) + ') ' + str(stock_qty3) +
                    ' EA : ' + str(current_price) + ' meets the buy condition!`')            
                cpTradeUtil.TradeInit()
                acc = cpTradeUtil.AccountNumber[0]      # 계좌번호
                accFlag = cpTradeUtil.GoodsList(acc, 1) # -1:전체,1:주식,2:선물/옵션                
                ...
                cpOrder.SetInputValue(2, accFlag[0]) # 상품구분 - 주식 상품 중 첫번째
                cpOrder.SetInputValue(3, code)       # 종목코드
                cpOrder.SetInputValue(4, stock_qty3)    # 매수할 수량
                cpOrder.SetInputValue(7, "1")        # 주문조건 0:기본, 1:IOC, 2:FOK
                cpOrder.SetInputValue(8, "12")       # 주문호가 1:보통, 3:시장가
                                                    # 5:조건부, 12:최유리, 13:최우선 
                # 매수 주문 요청
                ret = cpOrder.BlockRequest() 
                printlog('최유리 IOC 매수 ->', stock_name, code, stock_qty3, '->', ret)
                if ret == 4:
                    remain_time = cpStatus.LimitRequestRemainTime
                    printlog('주의: 연속 주문 제한에 걸림. 대기 시간:', remain_time/1000)
                    time.sleep(remain_time/1000) 
                ...
                time.sleep(2)
                stock_name = get_stock_balance(code)
                printlog('get_stock_balance :', stock_name, stock_qty3)
                if stock_qty == 0:
                    sell_List.remove(code)
                dbgout("`sell_stock("+ str(stock_name) + ' : ' + str(code) + 
                    ") -> " + str(stock_qty3) + " EA sell!" + "`")
        else:
            return False
    except Exception as ex:
...
if __name__ == '__main__': 
    ...
    codeList = instCpCodeMgr.GetGroupCodeList(3) #001 종합주가지수 002 대형(시가총액) 003 중형(시가총액) 004 소형(시가총액) 005 음식료품 006 섬유,의복 007 종이,목재 008 화학 009 의약품 010 비금속광물
    buyList = []
