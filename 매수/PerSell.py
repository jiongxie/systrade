...
tarketCodeList = instCpCodeMgr.GetGroupCodeList(3) #001 종합주가지수 002 대형(시가총액) 003 중형(시가총액) 004 소형(시가총액) 005 음식료품 006 섬유,의복 007 종이,목재 008 화학 009 의약품 010 비금속광물
...
def get_stock_balance(code):
    ...
    if code == 'ALL':
        ...
        stock_name = instCpCodeMgr.CodeToName(code)
        return stock_name, 0
...
def buy_etf(code):
    ...
    try:
        ...
        if  current_price > target_price and current_price > ma5_price \
            and current_price > ma10_price and cur_per == 1:  
...
def sell_stock(code):
    ...
    try:
        if code in sell_List: 
            ...
            #printlog('bought_list:', bought_list, 'len(bought_list):',
            #    len(bought_list), 'target_buy_count:', target_buy_count)     
            if target_price * 1.05 > current_price > target_price  or target_price * 0.95 < current_price < target_price * 0.97: 
            ...
            if target_price * 1.1 > current_price > target_price * 1.05 or target_price * 0.9 < current_price < target_price * 0.95: 
                printlog(stock_name + '(' + str(code) + ') ' + str(stock_qty2) +
                    ' EA : ' + str(current_price) + ' meets the sell condition!`')            
                cpTradeUtil.TradeInit()
                acc = cpTradeUtil.AccountNumber[0]      # 계좌번호
                ...
                dbgout("`sell_stock("+ str(stock_name) + ' : ' + str(code) + 
                    ") -> " + str(stock_qty2) + " EA sell!" + "`")
            if current_price > target_price * 1.1 or  current_price < target_price * 0.9: 
                printlog(stock_name + '(' + str(code) + ') ' + str(stock_qty2) +
                    ' EA : ' + str(current_price) + ' meets the sell condition!`')            
                cpTradeUtil.TradeInit()
                acc = cpTradeUtil.AccountNumber[0]      # 계좌번호
