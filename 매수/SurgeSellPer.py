...
slack = Slacker('xoxb-1659407313334-1651426433367-3PhkUL5ZaxDNb58wBSf27hdZ')
def dbgout(message):
...
def get_surge(instStockChart, code):
    instStockChart.SetInputValue(0, code)
    instStockChart.SetInputValue(1, ord('2'))
    instStockChart.SetInputValue(4, 60)
    instStockChart.SetInputValue(5, 8)
    ...
    if(volumes[0] > averageVolume * 2):
        return 1
    else:
        return 0

def get_Per(instMarketEye, code):
    ...
    for i in range(numStock):
        Per = instMarketEye.GetDataValue(0, i)
        Pers.append(Per)
        if 0 < Pers[i] < 50: #sumPer / numStock
            return 1
        else :
            return 0   
...
def buy_etf(code):
    ...
    try:
        ...
        surge_price = get_surge(instStockChart, code)
        cur_per = get_Per(instMarketEye, code)
        ...
        if target_price * 1.03 > current_price > target_price and current_price > ma5_price \
            and current_price > ma10_price and surge_price == 1 and cur_per == 1:  
...
def sell_stock(code):
    ...
    try:
        if code in sell_List: 
            time_now = datetime.now()
            current_price, ask_price, bid_price = get_current_price(code) 
            target_price = get_target_price(code)    # 매도 목표가
            stock_name, stock_qty = get_stock_balance(code)  # 종목명과 보유수량 조회
            stock_qty2 = stock_qty // 3
            if stock_qty2 == 0:
                stock_qty2 = stock_qty
            if stock_qty == 0:
                return False
            
            #printlog('bought_list:', bought_list, 'len(bought_list):',
            #    len(bought_list), 'target_buy_count:', target_buy_count)     
            if  current_price > target_price * 1.05  or  current_price < target_price * 0.97: 
                printlog(stock_name + '(' + str(code) + ') ' + str(stock_qty2) +
                    ' EA : ' + str(current_price) + ' meets the sell condition!`')            
                cpTradeUtil.TradeInit()
                acc = cpTradeUtil.AccountNumber[0]      # 계좌번호
                accFlag = cpTradeUtil.GoodsList(acc, 1) # -1:전체,1:주식,2:선물/옵션  

                # 최유리 FOK 매수 주문 설정
                cpOrder.SetInputValue(0, "1")        # 2: 매수
                cpOrder.SetInputValue(1, acc)        # 계좌번호
                cpOrder.SetInputValue(2, accFlag[0]) # 상품구분 - 주식 상품 중 첫번째
                cpOrder.SetInputValue(3, code)       # 종목코드
                cpOrder.SetInputValue(4, stock_qty2)    # 매도할 수량
                cpOrder.SetInputValue(7, "2")        # 주문조건 0:기본, 1:IOC, 2:FOK
                cpOrder.SetInputValue(8, "12")       # 주문호가 1:보통, 3:시장가
                                                    # 5:조건부, 12:최유리, 13:최우선 
                # 매도 주문 요청
                ret = cpOrder.BlockRequest() 
                printlog('최유리 FOK 매도 ->', stock_name, code, stock_qty2, '->', ret)
                if ret == 4:
                    remain_time = cpStatus.LimitRequestRemainTime
                    printlog('주의: 연속 주문 제한에 걸림. 대기 시간:', remain_time/1000)
                    time.sleep(remain_time/1000) 
                    return False
                time.sleep(2)
                stock_name = get_stock_balance(code)
                printlog('get_stock_balance :', stock_name, stock_qty2)
                if stock_qty == 0:
                    sell_List.remove(code)
                dbgout("`sell_stock("+ str(stock_name) + ' : ' + str(code) + 
                    ") -> " + str(stock_qty2) + " EA sell!" + "`")
            """                
            elif target_price * 1.1 > current_price > target_price * 1.07 or target_price * 0.9 < current_price < target_price * 0.95: 
                printlog(stock_name + '(' + str(code) + ') ' + str(stock_qty2) +
                    ' EA : ' + str(current_price) + ' meets the sell condition!`')            
                cpTradeUtil.TradeInit()
                acc = cpTradeUtil.AccountNumber[0]      # 계좌번호
            ...
            elif current_price > target_price * 1.1 or target_price * 0.95 < current_price < target_price * 0.9: 
                printlog(stock_name + '(' + str(code) + ') ' + str(stock_qty2) +
                    ' EA : ' + str(current_price) + ' meets the sell condition!`')            
                cpTradeUtil.TradeInit()
                acc = cpTradeUtil.AccountNumber[0]      # 계좌번호
        ...
        dbgout("`sell_stock("+ str(code) + ") -> exception! " + str(ex) + "`")
...
if __name__ == '__main__': 
    ...
    #codeList = instCpCodeMgr.GetGroupCodeList(4) #001 종합주가지수 002 대형(시가총액) 003 중형(시가총액) 004 소형(시가총액) 005 음식료품 006 섬유,의복 007 종이,목재 008 화학 009 의약품 010 비금속광물
    codeList = instCpCodeMgr.GetStockListByMarket(2) #1.코프피2.코스닥
    buyList = []
